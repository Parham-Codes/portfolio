  ...
  const filteredProjects = useMemo(() => {
    return projects.filter((project) => {
      ...
      const matchesSearch =
        ...
        (project.shortTitle && project.shortTitle.toLowerCase().includes(query)) ||
  ...
  return (
    <div className="py-8 sm:py-12 flex flex-col gap-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-6">
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-[#87929a] hover:text-[#38bdf8] font-sans text-xs sm:text-sm font-semibold transition-colors group w-fit"
        >
          <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
          <span>Back to Home</span>
        </Link>

        <div className="flex items-center gap-2 font-mono text-xs text-[#bdc8d1]">
          <Link to="/" className="hover:text-white transition-colors">
            Home
          </Link>
          <span className="text-[#3e484f]">/</span>
          <span className="text-[#38bdf8] font-semibold">Projects</span>
        </div>
      </div>
