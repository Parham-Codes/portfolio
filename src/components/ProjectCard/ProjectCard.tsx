...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, reversed = false }) => {
  ...
  const accentStyles = accentStyleMap[project.accent as keyof typeof accentStyleMap] || accentStyleMap.cyan;
  ...
  return (
    <>
      ...
      {/* Modal Preview — Comprehensive Technical Details */}
      {activeModal && (
        <div
        ...
        >
          <div
            className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-[#181c24] border border-white/20 p-5 sm:p-6 md:p-8 shadow-2xl flex flex-col gap-4 sm:gap-5 text-left"
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {(project.shortLabel || project.shortTitle) && (
                  <span className="font-mono text-xs text-[#87929a] hidden sm:inline">
                    ({project.shortLabel || project.shortTitle})
                  </span>
                )}
              </div>
              <button
                onClick={() => setActiveModal(false)}
                className="text-gray-400 hover:text-white text-xs sm:text-sm font-mono px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 shrink-0 transition-colors"
              >
                ESC [×]
              </button>
            </div>
